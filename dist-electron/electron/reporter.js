...
export function registerReportHandlers() {
    ipcMain.handle('bsc:generate-pdf', (_event_1, _a) => __awaiter(this, [_event_1, _a], void 0, function* (_event, { hosts, subnet }) {
        try {
            // Show save dialog
            ...
            doc.moveTo(50, doc.y + 4).lineTo(562, doc.y + 4).stroke(NAVY);
            doc.moveDown(0.5);
            for (const host of hosts.filter((h) => h.risk !== 'low')) {
                if (doc.y > 680)
                    doc.addPage();
                const rc = riskColor(host.risk);
            ...
            doc.fontSize(9).fillColor(WHITE).font('Helvetica')
                .text(`This automated scan identified ${high} high-risk and ${medium} medium-risk findings on your network. ` +
                'A Bravo Six certified assessment goes deeper — credentialed internal scanning, configuration review, ' +
                'policy analysis, and a written remediation roadmap delivered by a CISSP-certified SDVOSB consultant.', 65, ctaY + 32, { width: 480 });
            doc.fontSize(9).fillColor(GOLD).font('Helvetica-Bold')
                .text('info@bravosixcyber.com  ·  656.245.8307  ·  bravosixcyber.com  ·  SDVOSB  ·  CISSP  ·  CICP', 65, ctaY + 85, { width: 480 });
            // ── FOOTER ──
            ...
            for (let i = 0; i < pageCount; i++) {
                ...
                doc.fontSize(7).fillColor(GRAY).font('Helvetica')
                    .text(`BSC Network Scout  ·  Bravo Six Cyber LLC  ·  CONFIDENTIAL  ·  Page ${i + 1} of ${pageCount}`, 50, 771, { width: 512, align: 'center' });
            }
            doc.end();
